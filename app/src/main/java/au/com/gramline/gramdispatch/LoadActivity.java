...
public class LoadActivity extends AppCompatActivity {
        ...
        saveOrderButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                for (int i = 0; i < size; i++)
                {
                    ...
                    EditText qtyLoaded = tableLayout.findViewWithTag("qtyLoaded_" + i);
                    if (qtyLoaded.length() != 0)
                    {
                        String qtyLoadedValue = qtyLoaded.getText().toString();
                        savedOrder.results.get(i).QtyLoaded = Integer.parseInt(qtyLoadedValue);
                    }
                }
    ...
    public CollectedOrderList getOrderFromFile(CollectedOrderList collectedOrderList)
    {
        ...
        for (CollectedOrderList.CollectedOrder collectedOrder : collectedOrderList.results) {
            ...
            // Add a TextView in the eighth column.
            EditText qtyLoaded = new EditText(context);
            if (collectedOrder.QtyLoaded != null)
            {
                qtyLoaded.setText(String.valueOf(collectedOrder.QtyLoaded));
            }
            tableRow.addView(qtyLoaded, 7, layoutParams);
            qtyLoaded.setTag("qtyLoaded_" + size);
            qtyLoaded.setInputType(InputType.TYPE_CLASS_NUMBER);
            size++;
